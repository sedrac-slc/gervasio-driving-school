    ...
    <div id="accordion-collapse" data-accordion="collapse" class="bg-white rounded-lg">
        ...
        <div id="accordion-collapse-body-2" class="hidden" aria-labelledby="accordion-collapse-heading-2">
            <div class="flex flex-col md:flex-row gap-2 items-center justify-between m-2">
                <x-input-search href="{{ route('students.search') }}" />
                <div class="flex gap-1">
                    <x-link-report-pdf href="{{ route('report.student') }}" label="Exporta"/>
                    <x-button-create href="{{ route('students.store') }}"/>
                </div>
            </div>
            <div class="relative overflow-x-auto shadow-md my-3">
                    ...
                    <tbody>
                        @foreach ($students as $student)
                            <tr
                                ...
                                <th scope="row" id="table-name-{{ $student->id }}"
                                    class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $student->user->name }}
                                </th>
                                <td class="px-6 py-4" id="table-email-{{ $student->id }}">
                                    {{ $student->user->email }}
                                </td>
                                <td class="px-6 py-4 text-center flex items-center gap-10 justify-center">
                                    <x-link-edit href="{{ route('students.update', $student->id) }}" redirect
                                        json="{{ $student->id }}" />
                ...
                @include('components.custom.table-links', ['links' => $students])
            </div>
        </div>
    </div>
