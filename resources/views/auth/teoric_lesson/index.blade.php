    ...
    <div id="accordion-collapse" data-accordion="collapse" class="bg-white rounded-lg">
        ...
        <div id="accordion-collapse-body-2" class="hidden" aria-labelledby="accordion-collapse-heading-2">
            <div class="flex flex-col md:flex-row gap-2 items-center justify-between m-2">
                <x-input-search href="{{ route('teoric_lessons.search') }}" />
                <x-button-create href="{{ route('teoric_lessons.store') }}" />
            </div>
            <div class="relative overflow-x-auto shadow-md my-3">
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        @foreach ($teoricLessons as $teoricLesson)
                            <tr
                                ...
                                <td class="px-6 py-4" id="table-enrolment-code-{{ $teoricLesson->id }}"
                                    data-id="{{ $teoricLesson->enrolment->id }}">
                                    {{ $teoricLesson->enrolment->code }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $teoricLesson->enrolment->student->user->name }}
                                </td>
                                <td class="px-6 py-4 text-center flex items-center gap-10 justify-center">
                                    <x-link-edit href="{{ route('teoric_lessons.update', $teoricLesson->id) }}" redirect
                                        json="{{ $teoricLesson->id }}" />
                                    <x-link-delete href="{{ route('teoric_lessons.destroy', $teoricLesson->id) }}" />
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                @include('components.custom.table-links', ['links' => $teoricLessons])
            </div>
        </div>
    </div>
