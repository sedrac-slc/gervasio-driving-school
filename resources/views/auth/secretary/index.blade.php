    ...
    <div id="accordion-collapse" data-accordion="collapse" class="bg-white rounded-lg">
        ...
        <div id="accordion-collapse-body-2" class="hidden" aria-labelledby="accordion-collapse-heading-2">
            <div class="flex flex-col md:flex-row gap-2 items-center justify-between m-2">
                <x-input-search href="{{ route('secretaries.search') }}" />
                <div class="flex gap-1">
                    <x-link-report-pdf href="{{ route('report.secretary') }}" />
                    <x-button-create href="{{ route('secretaries.store') }}" />
                </div>
            </div>
            <div class="relative overflow-x-auto shadow-md my-3">
                    ...
                    <tbody>
                        @foreach ($secretaries as $secretary)
                            <tr
                                ...
                                <th scope="row" id="table-name-{{ $secretary->id }}"
                                    class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $secretary->user->name }}
                                </th>
                                <td class="px-6 py-4" id="table-email-{{ $secretary->id }}">
                                    {{ $secretary->user->email }}
                                </td>
                                <td class="px-6 py-4 text-center flex items-center gap-10 justify-center">
                                    <x-link-edit href="{{ route('secretaries.update', $secretary->id) }}" redirect
                                        json="{{ $secretary->id }}" />
                ...
                @include('components.custom.table-links', ['links' => $secretaries])
            </div>
        </div>
    </div>
